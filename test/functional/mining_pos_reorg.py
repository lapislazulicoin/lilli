...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected LiLLi and zLiLLi supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zLiLLisupply']:
                assert_equal(node['zLiLLisupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check LiLLi and zLiLLi supply at the beginning
        # ------------------------------------------
        # zLiLLi supply: 2 coins for each denomination
        ...
        # LiLLi supply: block rewards minus burned fees for minting
        ...
        # Verify that LiLLi and zLiLLi supplies were properly updated after the spends and reorgs
        self.log.info("Check LiLLi and zLiLLi supply...")
