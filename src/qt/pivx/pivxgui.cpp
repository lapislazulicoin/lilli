...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("LapisLazuli client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("LapisLazuli"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("LapisLazuli"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
